...
<div class="card p-4">
    ...
    <form action="/appointments" method="POST">
        ...
        <select name="pet" class="form-select mb-3" required>
            ...
            @foreach($pets as $pet)
                <option value="{{ $pet->name }}" {{ old('pet') == $pet->name ? 'selected' : '' }}>
                    {{ $pet->name }}
                </option>
            @endforeach
        </select>
        ...
        <select name="owner" class="form-select mb-3" required>
            ...
            @foreach($owners as $owner)
                <option value="{{ $owner->name }}" {{ old('owner') == $owner->name ? 'selected' : '' }}>
                    {{ $owner->name }}
                </option>
            @endforeach
        </select>
        ...
        <div class="row g-2 mb-3">
            ...
            <div class="col-4">
                <input type="number" name="appointment_year" value="{{ old('appointment_year', date('Y')) }}" class="form-control" placeholder="Año (Ej: 2015)" min="1900" max="2100" required>
            </div>
        </div>

        <label class="form-label fw-bold">Motivo de la Consulta</label>
        <input type="text" name="reason" value="{{ old('reason') }}" class="form-control mb-3" placeholder="Ej: Control de vacunas">
